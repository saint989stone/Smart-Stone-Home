...
function main(){
    ...
    $(this).children().filter(".lvl1Gnrl").slideUp(500);
...
function mainActv(){
    ...
    $(this).children().filter(".lvl1Gnrl").slideDown(500);
    lvl1GnrlActv(this);
}
...
//Функция активации Блока меню "Размещения" в зависимости от значения элемента Json swtcGnrl
function lvl1GnrlActv(mainElem) {
    swtcGnrlStrg_lvl1Gnrl = "swtcGnrl" + String($(mainElem).attr("id") + "Swtc"); //формируем имя GnrlSwtc
    loclElemStrg_lvl1Gnrl = swtcGnrlStrg_lvl1Gnrl.slice(0, 8); //формируем имя соответствующего JSON массива
    loclElem = window[loclElemStrg_lvl1Gnrl]; //получаем JSON массив
    swtcActv(loclElemStrg_lvl1Gnrl);
    if (loclElem[swtcGnrlStrg_lvl1Gnrl] == 0){ //дезактивируем соответствующий блок lvl1
        $(mainElem).children().filter(".lvl1").slideUp(500);
    }
    else if (loclElem[swtcGnrlStrg_lvl1Gnrl] == 1) { //активируем соответствующий блок lvl1
...
//Функция активации Блока меню "Размещения" в зависимости от значения элемента Json swtcGnrl
function lvl1GnrlActv(mainElem) {
    swtcGnrlStrg_lvl1Gnrl = "swtcGnrl" + String($(mainElem).attr("id") + "Swtc");
    loclElemStrg_lvl1Gnrl = swtcGnrlStrg_lvl1Gnrl.slice(0, 8);
    loclElem = window[loclElemStrg_lvl1Gnrl];
    swtcActv(loclElemStrg_lvl1Gnrl);
    if (loclElem[swtcGnrlStrg_lvl1Gnrl] == 0){
        $(mainElem).children().filter(".lvl1").slideUp(500);
        }
    else if (loclElem[swtcGnrlStrg_lvl1Gnrl] == 1){
        $(mainElem).children().filter(".lvl1").slideDown(500);
    }
}
...
function swtcBoxxActv(swtcBoxx_SwtcBoxx, loclElemStrg_SwtcBoxx){
    if (["clmt", "scrt", "fire", "watr", "admr"].indexOf(loclElemStrg_SwtcBoxx.slice(0, 4)) != -1){ //проверяем выключатель на соответствие к определенному блоку системы (lght, scrt, и т.д)
     ...
     indcElemBlck_IndcActv.each(function () { //перебираем элементы indcBlck
         ...
         if (indcElemClss_IndcActv == "indc"){
            if (loclElem_IndcActv[swtcName_IndcActv] == 1 && loclElem_IndcActv[indcName_IndcActv] == 0){ //если Swtc системы включен и нет тревоги
                ...
                main_IndcActv.animate({backgroundColor:"#2b2b2b"}, 500); //в случае тревоги родительские элементы окрашены черным
