function handSwtc (swtcName_HandSwtc, loclElemStrg_HandSwtc){
    ...
    if (swtcName_HandSwtc.slice(8) == "SwtcSwtc"){ //проверка SwtcSwtc на значение SwtcGnrl если SwtcGnrl = 0, то соответсвующий SwtcSwtc не активен
        if (window["swtcGnrl"]["swtcGnrl"+swtcName_HandSwtc.slice(0, 4)+"Swtc"] == 0){
            window[loclElemStrg_HandSwtc][swtcName_HandSwtc] = 0;
        }
    }
    if (loclElemStrg_HandSwtc == "swtcGnrl"){
        swtcAreaGnrl = []; //создаем пустой массив для передачи в Функцию управления
        loclElem = ["Ktch", "Hall"]; //массив содержащий все Locl
        for (i = 0; i <= loclElem.length - 1; i ++){ //цикл формирующий массив из Элементов SwtcSwtc
            swtcAreaGnrl.push(swtcName_HandSwtc.slice(8, 12) + loclElem[i] + "SwtcSwtc"); //формируем массив из элементов типа lghtHallSwtcSwtc
        }
        handSwtcGnrl(swtcName_HandSwtc, swtcAreaGnrl);
    }
    else if (loclElemStrg_HandSwtc == "lghtHall"){ //проверяем расположение элемента выключателя
        if (lghtHall["lghtHallSwtcSwtc"] == 0 || swtcGnrl["swtcGnrllghtSwtc"] == 0){
        ...
        if (lghtKtch["lghtKtchSwtcSwtc"] == 0 || swtcGnrl["swtcGnrllghtSwtc"] == 0){ //проверяем состояние главного Swtc`а
...
function handSwtcGnrl(swtcGnrlName_HandSwtcGnrl, swtcArea_HandSwtcGnrl){
    for (i = 0; i <= swtcArea_HandSwtcGnrl.length - 1; i ++){ //цикл перебора элементов SwtcSwtc
        if (swtcArea_HandSwtcGnrl[i] == "watrHallSwtcSwtc"){
            continue;
        }
        swtcBoxx_HandSwtcGnrl = ($(window[swtcArea_HandSwtcGnrl[i]]).parent().parent());
        loclElemStrg_HandSwtcGnrl = swtcArea_HandSwtcGnrl[i].slice(0, 8);
        loclElem_HandSwtcGnrl = window[loclElemStrg_HandSwtcGnrl];
         if (swtcGnrl[swtcGnrlName_HandSwtcGnrl] != loclElem_HandSwtcGnrl[swtcArea_HandSwtcGnrl[i]]){
             if (swtcGnrl[swtcGnrlName_HandSwtcGnrl] == 0){ //проверяем значение  элемента в swtcGnrl
                loclElem_HandSwtcGnrl[swtcArea_HandSwtcGnrl[i]] = 0; //изменяем значение элемента в Json на 0
            }
            else if (swtcGnrl[swtcGnrlName_HandSwtcGnrl] == 1) {
                loclElem_HandSwtcGnrl[swtcArea_HandSwtcGnrl[i]] = 1; //изменяем значение элемента в Json на 1
            }
            swtcBoxxActv(swtcBoxx_HandSwtcGnrl, loclElemStrg_HandSwtcGnrl); //Функция активации SwtcBoxx в зависимости от блока к которому относится Swtc
            lvl1GnrlActv(window[loclElemStrg_HandSwtcGnrl.slice(0, 4)]); ///Функция активации Блока меню "Размещения" в зависимости от значения элемента Json swtcGnrl
            setTimeout(sendLoclElem(loclElemStrg_HandSwtcGnrl, loclElem_HandSwtcGnrl), 4); //отправляем Json массивы на WEB сервер
        }
    }
}
